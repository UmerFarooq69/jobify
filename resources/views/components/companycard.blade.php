<a href="{{ route('company.jobs', $company->id) }}" class="block">
    <div class="p-6 bg-white shadow-md rounded-lg flex flex-col text-left mx-4 hover:bg-gray-200 transition-all duration-100">
        <div class="flex items-center mb-4">
            <img src="{{ asset('storage/' . $company->image) }}" alt="Company Image" class="w-[70px] h-[70px] object-contain rounded-md shadow-lg mr-4">
            <h1 class="text-xl font-semibold"><span class="font-bold">{{$company->name}}</span></h1>
        </div>    
        <h2 class="text-xl font-semibold text-gray-700">{{ \Illuminate\Support\Str::limit($company->description, 170) }}</h2>
