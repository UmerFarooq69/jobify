<x-form>
    <div class="min-h-full flex flex-col items-center justify-center bg-gradient-to-r from-blue-900 to-blue-100 p-8">
        <div
            class="bg-white p-8 rounded-lg shadow-xl w-full max-w-4xl transform hover:scale-105 transition-transform duration-300">

            <div class="flex justify-end">
                <a href="{{ url()->previous() }}"
                    class="bg-blue-500 text-white hover:bg-blue-600 hover:text-white font-semibold py-3 px-8 mt-2 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    Go Back
                </a>
            </div>
            <h2 class="text-2xl font-bold text-gray-900 mt-4">Apply for {{ $job->job_title }}</h2>
            <p class="text-gray-700 mb-6">Company: <span
                    class="font-semibold text-gray-800">{{ $job->company->name }}</span></p>
