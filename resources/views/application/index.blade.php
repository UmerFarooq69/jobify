<x-admin>
    ...
    <div class="mb-6 text-lg font-semibold text-gray-800">
        @if(request('company_id'))
            Showing {{ $applications->where('job.company_id', request('company_id'))->count() }} applications for
            {{ $companies->firstWhere('id', request('company_id'))->name }}
        @else
    ...
    @if($applications->isEmpty())
        <div class="flex flex-col items-center justify-center p-10 bg-gray-50 border border-gray-200 rounded-lg">
            <svg class="w-16 h-16 text-gray-400 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M9 17v-6a3 3 0 013-3h1m6 0a3 3 0 00-3-3H6a3 3 0 00-3 3v12a3 3 0 003 3h9a3 3 0 003-3v-6" />
            </svg>
            ...
            @if(request('company_id') == null || $application->job->company->id == request('company_id'))
                <div class="border border-gray-300 p-4 my-4 rounded-lg bg-gray-50 shadow">
                    ...
                    <a href="{{ asset('storage/' . $application->cv) }}" download
                        ...
                        <svg class="w-5 h-5 text-blue-500 hover:text-blue-700 transition" xmlns="http://www.w3.org/2000/svg"
